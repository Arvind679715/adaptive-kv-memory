...
\begin{itemize}
    \item \textbf{Models}: TinyLlama-1.1B-Chat-v1.0 (22L, 4 KV heads, GQA), Llama-2-7B (32L, 32 KV heads, MHA), and Qwen2.5-0.5B (24L, 2 KV heads, GQA)
    ...
    \item \textbf{Baseline methods}: Full cache (FP16), H2O, SnapKV, KIVI 2-bit (FIFO demotion)
    \item \textbf{AKV default config}: Importance-aware demotion with $n_{\text{anchors}}=16$, decay $\lambda=0.3$, last-query-position scoring, min-max symmetric quantization (group\_size=32)
\end{itemize}
...
AKV min-max 4b/2b & 5.91 & 4/2 & $\sim$3.5 & +1.7\% \\
\textbf{NormQuant 3b/3b} & \textbf{6.00} & \textbf{3} & \textbf{$\sim$3.5} & \textbf{+3.2\%} \\
NormQuant 2b/2b & 6.48 & 2 & $\sim$2.5 & +11.5\% \\
\midrule
AKV min-max 2b/2b & 9.71 & 2 & 2 & +67\% \\
KIVI 2-bit & 12.33 & 2 & 2 & +112\% \\
SnapKV (budget=128) & 6.83 & --- & --- & +17.4\% \\
H2O (budget=128) & 44.22 & --- & --- & +661\% \\
\bottomrule
\end{tabular}
\caption{WikiText-2 perplexity comparison on TinyLlama-1.1B (2048 tokens). NormQuant 3b matches min-max 4b quality at 25\% fewer bits. AKV rows use FIFO demotion; the full system with importance-aware demotion adds an additional +0.97\% improvement at 4-bit (Table~\ref{tab:importance_vs_fifo}).}
\label{tab:main_results}
\end{table}

\paragraph{Key findings:}
\begin{enumerate}
    \item NormQuant 3b/3b (6.00) is within 0.09 PPL of min-max 4b (5.91) while using $\sim$3.5 effective bits vs 4 bits---a 1.14$\times$ compression improvement at comparable quality.
    \item NormQuant 4b/4b (5.89) \textit{beats} min-max 4b (5.91), demonstrating that the rotation + per-group norm approach achieves better quality at the same bit budget.
    \item At 2 bits, NormQuant (6.48) achieves 33\% lower PPL than min-max (9.71) and 47\% lower than KIVI (12.33).
...
\toprule
\textbf{Hot Budget} & \textbf{\% FP16} & \textbf{PPL} & \textbf{$\Delta$\%} & \textbf{Migrations} & \textbf{Warm Bits} \\
\midrule
2048 (Full) & 100\% & 5.81 & --- & 0 & --- \\
512 & 25\% & 5.86 & +0.8\% & 0 & 3b \\
256 & 12.5\% & 5.86 & +0.8\% & 0 & 3b \\
128 & 6.25\% & 5.86 & +0.8\% & 0 & 3b \\
\textbf{64} & \textbf{3.1\%} & \textbf{5.86} & \textbf{+0.8\%} & 0 & 3b \\
\midrule
128 & 6.25\% & 5.86 & +0.8\% & 0 & 2b \\
64 & 3.1\% & 5.86 & +0.8\% & 0 & 2b \\
\bottomrule
\end{tabular}
\caption{ProductionCache PPL vs hot budget with NormQuant warm tier and importance-aware demotion. The combination of importance scoring + NormQuant achieves near-lossless quality (+0.8\%) across all hot budgets and bit-widths. At 2048 tokens, the system's importance-aware page management prevents migration triggering, keeping all tokens in the hot tier. Longer contexts would see progressive warm-tier usage.}
\label{tab:hot_budget}
\end{table}

\paragraph{Key insight:} At 2048-token context, the ProductionCache achieves only +0.8\% PPL degradation regardless of hot budget or warm bit-width. The importance scoring system combined with chunked evaluation means the cache infrastructure adds minimal overhead. The zero-migration behavior at this sequence length indicates that the system's page-based allocation efficiently retains tokens without forced demotion---longer sequences (4K+) would trigger progressive migration to the warm tier, where NormQuant quality (Table~\ref{tab:main_results}) would determine the PPL floor.
...
AKV min-max 2b & 9.71 & +67\% & --- \\
KIVI 2-bit & 12.33 & +112\% & +27\% \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{RULER degradation at long context}: As shown in Table~\ref{tab:ruler}, AKV's generative retrieval accuracy degrades at 4K+ when the hot budget is a small fraction of total context. Adaptive hot budget scaling (e.g., $\text{hot} = \max(512, 0.25 \times N)$) or importance-pinning of high-attention outlier tokens could address this.
...
Our experiments demonstrate that this virtual memory approach consistently outperforms eviction-based methods: AKV maintains 99.6\% retrieval accuracy at all context depths on passkey retrieval while extending maximum context by 3--5$\times$ on memory-constrained hardware. On LongBench, AKV matches full-cache quality ($-$0.5\% overall) while H2O degrades by $-$10.3\%, confirming that information retention translates directly to downstream task performance. On the RULER multi-task retrieval benchmark, AKV dominates H2O at every context length (0.94 vs 0.36 at 1K) and is the only method that operates at 16K where full cache OOMs---though generative retrieval accuracy at high compression ratios (hot covers $<$15\% of context) remains an area for improvement. At 32K context, the bounded working set achieves 10.4$\times$ decode throughput over full-cache attention while retaining 100\% of tokens---H2O achieves higher raw speed (50$\times$) but permanently discards 97\% of context, collapsing needle retrieval to 1.2\%. The production cache confirms practical feasibility with 3.93ms p50 decode latency and zero-allocation operation, with a drop-in API compatible with HuggingFace and FlashAttention.
