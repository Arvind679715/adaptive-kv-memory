...
\subsection{Full WikiText-2 Evaluation (291K Tokens)}
\label{sec:full_wikitext}

The preceding triage experiments use short multi-passage contexts ($\sim$5K tokens) to isolate component contributions. We now evaluate AKV on the \textbf{full WikiText-2 test set} (291,608 tokens) with sliding-window perplexity on Qwen2.5-1.5B-Instruct, which stresses the warm tier over hundreds of demotion cycles:

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{PPL} & \textbf{$\Delta$\% vs FP16} \\
\midrule
FP16 (DynamicCache) & 8.70 & --- \\
AKV block-affine 4-bit & 8.84 & +1.67\% \\
AKV block-affine 3-bit & 9.75 & +12.05\% \\
\midrule
\multicolumn{3}{l}{\textit{Published comparisons (same eval on similar models):}} \\
KIVI 4-bit (published) & 5.89 & +1.4\% \\
KIVI 2-bit (published) & 12.33 & +112\% \\
\bottomrule
\end{tabular}
\caption{WikiText-2 sliding-window perplexity on Qwen2.5-1.5B-Instruct (291,608 tokens). AKV 4-bit achieves only +1.67\% degradation over FP16 baseline, competitive with published KIVI-4bit results on comparable models. The 3-bit gap widens from +3.1\% (short context) to +12.05\% (full corpus) as quantization noise accumulates over many demotion cycles.}
\label{tab:full_wikitext}
\end{table}

\paragraph{Context-length scaling.} The 3-bit gap increases from +3.1\% (5K context, Table~\ref{tab:quantizer_comparison}) to +12.05\% (291K tokens). This is expected: with a sliding window of 2048 tokens and hot budget of 256, 87.5\% of tokens are quantized at any point. Over 291K tokens, the warm tier accumulates thousands of quantized KV entries and dequantization errors compound. The 4-bit configuration shows only +1.67\% degradation even at full scale, confirming that 4-bit block-affine quantization is production-ready for long-context workloads. For applications requiring 3-bit compression, adaptive hot budget scaling (increasing hot tier as context grows) is a promising direction.
...
The warm tier stores quantized tensors as packed integer codes plus per-axis scale/zero side information:

\begin{verbatim}
_k_codes:  (num_heads, max_seq_len, head_dim)         uint8
_k_scale:  (num_heads, 1, head_dim)                   fp16
_k_zero:   (num_heads, 1, head_dim)                   fp16
_v_codes:  (num_heads, max_seq_len, head_dim)         uint8
